...
class PerformanceTestUtils:
    ...
    def save_test_results(environment) -> Dict[str, Any]:
        ...
        with open(csv_file, "a", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=test_results.keys(), extrasaction="ignore"
            )
    ...
    def _plot_performance_metrics(
        test_results: Dict[str, Any], output_file: Path
    ) -> None:
        """パフォーマンスメトリクスをプロット
        ...
        try:
            ...
            thresholds = [
                test_results["thresholds"]["response_time"][m] for m in metrics
            ]
            ...
            logger.warning(
                "matplotlib is not installed. Skipping performance plot generation."
            )
