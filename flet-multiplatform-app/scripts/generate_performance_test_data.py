...
class PerformanceDataGenerator:
    ...
    def generate_users(self, count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(1, count + 1):
            user = {
                ...
                "role": random.choices(
                    self.user_roles, weights=[0.05, 0.15, 0.75, 0.05], k=1  # 確率を調整
                )[0],
                "metadata": {
    ...
    def generate_products(self, count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(1, count + 1):
            ...
            product = {
                ...
                "tags": random.sample(
                    ["sale", "new", "popular", "limited"], k=random.randint(0, 3)
                ),
                "metadata": {
    ...
    def generate_orders(
        self, count: int, customer_count: int, product_count: int
    ) -> List[Dict[str, Any]]:
        """注文データを生成"""
        ...
        for i in range(1, count + 1):
            ...
            order = {
                ...
                "metadata": {
                    "discount": (
                        round(random.uniform(0, 0.3) * total_amount, 2)
            ...
            if status in ["shipped", "delivered"]:
                ...
                if status == "delivered":
                    order["delivered_date"] = self.generate_date(
                        order["shipped_date"], "2023-12-31"
                    )
    ...
    def generate_transactions(
        self, count: int, customer_count: int
    ) -> List[Dict[str, Any]]:
        """取引データを生成"""
        ...
        for i in range(1, count + 1):
            ...
            transaction = {
                ...
                "customer_id": (
                    random.randint(1, customer_count) if customer_count > 0 else None
                ),
                ...
                "metadata": {
                    ...
                    "fee": round(
                        abs(amount) * random.uniform(0.01, 0.03), 2
                    ),  # 1-3% fee
                },
            }
    ...
    def generate_all(self, dataset_size: str = "small") -> Dict[str, str]:
        ...
        if dataset_size not in self.configs:
            raise ValueError(
                f"Invalid dataset size. Choose from: {', '.join(self.configs.keys())}"
            )
        ...
        transactions = self.generate_transactions(
            config["transactions"], len(customers)
        )
        ...
        logger.info(
            f"Test data generation completed. Metadata saved to {metadata_path}"
        )
        return result
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="パフォーマンステスト用のテストデータを生成"
    )
    parser.add_argument(
        "--size",
        type=str,
