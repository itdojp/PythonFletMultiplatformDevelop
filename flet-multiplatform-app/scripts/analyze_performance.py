...
class PerformanceAnalyzer:
            ...
            summary.update(
                {
                    ...
                    "total_duration_seconds": metrics["timestamp"].max()
                    - metrics["timestamp"].min(),
    ...
    def generate_html_report(self, test_type: str) -> None:
        ...
        """.format(
            ...
            """
                if "fail_ratio" in self.load_metrics(test_type).columns
                else ""
            ),
        )
...
def main():
    ...
    parser.add_argument(
        "--reports-dir", default="reports", help="Directory to save analysis reports"
    )
    parser.add_argument(
        "--test-type",
