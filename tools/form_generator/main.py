...
def main(page: ft.Page):
        ...
        tabs=[
            ft.Tab(
                ...
                content=ft.Container(
                    content=create_templates_view(form_templates),
                    padding=10
                ),
            ),
            ft.Tab(
                ...
                content=ft.Container(
                    content=create_custom_form_builder(form_fields),
                    padding=10
                ),
            ),
            ft.Tab(
                ...
                content=ft.Container(
                    content=create_form_settings_view(),
                    padding=10
                ),
            ),
            ft.Tab(
    ...
    preview_container = ft.Container(
        content=ft.Text("フォームを選択または作成すると、ここにプレビューが表示されます。"),
        padding=20,
    ...
    def create_templates_view(templates):
        ...
        for template in templates:
            card = ft.Card(
                content=ft.Container(
                    content=ft.Column([
                        ft.Text(template["name"], size=16, weight=ft.FontWeight.BOLD),
                        ft.Text(f"フィールド数: {len(template['fields'])}", size=12, color=ft.colors.BLACK54),
                        ft.Row([
                            ft.ElevatedButton("選択", on_click=lambda _, t=template: select_template(t)),
                            ft.OutlinedButton("プレビュー", on_click=lambda _, t=template: preview_template(t))
                        ])
                    ]),
                    padding=10
                ),
        ...
        return ft.Column([
            ...
            ft.Divider(),
            ft.ResponsiveRow(
                [ft.Column([card], col={"sm": 6, "md": 4, "xl": 3}) for card in template_cards]
            )
        ])
    ...
    def create_custom_form_builder(field_definitions):
        ...
        custom_fields_list = ft.ListView(
            spacing=2,
            height=300,
            width=500
        )
        ...
        return ft.Column([
            ...
            custom_fields_list,
            ft.Row([
                ft.ElevatedButton("プレビュー", icon=ft.icons.VISIBILITY, on_click=lambda _: preview_custom_form()),
                ft.FilledTonalButton("クリア", icon=ft.icons.CLEAR_ALL, on_click=lambda _: clear_custom_fields())
            ])
        ])
    ...
    def create_form_settings_view():
        ...
        submit_text = ft.TextField(
            label="送信ボタンテキスト",
            value=form_settings["submit_text"]
        )
        
        cancel_text = ft.TextField(
            label="キャンセルボタンテキスト",
            value=form_settings["cancel_text"]
        )
        
        show_cancel = ft.Checkbox(
            label="キャンセルボタンを表示",
            value=form_settings["show_cancel"]
        )
        ...
        responsive = ft.Checkbox(
            label="レスポンシブレイアウト",
            value=form_settings["responsive"]
        )
        ...
        preview_form = ft.Column(
            controls=[
                ...
                ft.Row([
                    ...
                    ft.OutlinedButton(form_settings["cancel_text"], visible=form_settings["show_cancel"])
                ])
            ],
            spacing=10
        )
    ...
    def add_custom_field(field_type, name, label, required):
        ...
        page.snack_bar = ft.SnackBar(
            content=ft.Text(f"フィールド「{label}」が追加されました"),
            action="閉じる"
        )
        page.snack_bar.open = True
        page.update()
    ...
    def clear_custom_fields():
        ...
        page.snack_bar = ft.SnackBar(
            content=ft.Text("すべてのフィールドがクリアされました"),
            action="閉じる"
        )
        page.snack_bar.open = True
        page.update()
            ...
            if field_type == "text":
                ...
                for option in field.get("options", []):
                    if isinstance(option, dict):
                        options.append(ft.dropdown.Option(option["value"], option["text"]))
                    else:
    ...
    def generate_form_code():
        if not selected_template and not custom_form_fields:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("テンプレートを選択するか、カスタムフォームにフィールドを追加してください"),
                action="閉じる"
            )
            page.snack_bar.open = True
            page.update()
        ...
        # 使用するフィールド定義
        fields = selected_template["fields"] if selected_template else custom_form_fields
...
def generate_code(form_name, fields, settings):
    ...
    for field in fields:
        ...
        if field_type == "text":
            ...
            if field.get("hint_text"):
                field_init.append(f"            hint_text=\"{field.get('hint_text')}\",")
            ...
            if options:
                ...
                for option in options:
                    if isinstance(option, dict):
                        field_init.append(f"                ft.dropdown.Option(\"{option['value']}\", \"{option['text']}\"),")
                    else:
                        field_init.append(f"                ft.dropdown.Option(\"{option}\"),")
                field_init.append("            ],")
            ...
            if field.get("content"):
                ...
                field_init.append("                ft.Radio(value=\"option1\", label=\"オプション1\"),")
                field_init.append("                ft.Radio(value=\"option2\", label=\"オプション2\"),")
            field_init.append("            ]),")
            if "value" in field:
                field_init.append(f"            value=\"{field.get('value')}\",")
            field_init.append("        )")
            ...
            field_init.append(f"        self.{field_name}_text = ft.Text(\"{field.get('label', '')}\")")
    ...
    if settings["show_cancel"]:
        buttons_init.append(f"        self.cancel_button = ft.OutlinedButton(\"{settings['cancel_text']}\", on_click=self.handle_cancel)")
    
    # フィールド登録とマッピング
    register_fields = ["        # フィールドの登録"]
    for field in fields:
        field_name = field["name"]
        register_fields.append(f"        self.fields[\"{field_name}\"] = self.{field_name}")
            ...
            for validator in field.get("validators", []):
                if isinstance(validator, str) and validator == "required":
                    if field["type"] in ["text", "dropdown"]:
                        validate_method.append(f"        if not self.{field_name}.value:")
                        validate_method.append(f"            errors.append(f\"{field_label}は必須です\")")
                        validate_method.append(f"            is_valid = False")
                    elif field["type"] in ["date", "time", "file"]:
                        validate_method.append(f"        if not self.{field_name}_value:")
                        validate_method.append(f"            errors.append(f\"{field_label}は必須です\")")
                        validate_method.append(f"            is_valid = False")
                
                elif isinstance(validator, str) and validator.startswith("min_length:"):
                    min_val = validator.split(":")[1].strip()
                    validate_method.append(f"        if self.{field_name}.value and len(self.{field_name}.value) < {min_val}:")
                    validate_method.append(f"            errors.append(f\"{field_label}は{min_val}文字以上で入力してください\")")
                    validate_method.append(f"            is_valid = False")
                
                elif isinstance(validator, str) and validator == "email":
                    validate_method.append(f"        if self.{field_name}.value and not re.match(r'^[\\w\\.-]+@[\\w\\.-]+\\.\\w+$', self.{field_name}.value):")
                    validate_method.append(f"            errors.append(\"有効なメールアドレスを入力してください\")")
                    validate_method.append(f"            is_valid = False")
                
                elif isinstance(validator, str) and validator.startswith("match:"):
                    other_field = validator.split(":")[1].strip()
                    validate_method.append(f"        if self.{field_name}.value != self.{other_field}.value:")
                    validate_method.append(f"            errors.append(f\"{field_label}が一致しません\")")
                    validate_method.append(f"            is_valid = False")
    ...
    for field in fields:
        ...
        if field["type"] in ["date", "time", "file"]:
            handler_methods.append(f"            data[\"{field_name}\"] = self.{field_name}_value")
        else:
            handler_methods.append(f"            data[\"{field_name}\"] = self.{field_name}.value")
    ...
    if settings["layout"] == "column":
        ...
        if settings["responsive"]:
            ...
            for field in fields:
                field_name = field["name"]
                if field["type"] in ["date", "time", "file"]:
                    build_method.append(f"        row_controls.append(ft.Column([self.{field_name}_text, self.{field_name}], col={{\"sm\": 12, \"md\": 6, \"lg\": 4}}))")
                else:
                    build_method.append(f"        row_controls.append(ft.Column([self.{field_name}], col={{\"sm\": 12, \"md\": 6, \"lg\": 4}}))")
    ...
    if settings["layout"] == "column":
        for field in fields:
            field_name = field["name"]
            if field["type"] in ["date", "time", "file"]:
                build_method.append(f"                ft.Column([self.{field_name}_text, self.{field_name}]),")
            else:
