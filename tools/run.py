...
def main(page: ft.Page):
    ...
    def launch_tool(e, tool_info):
        ...
        try:
            ...
            print(f"Starting tool: {tool_info['name']}")
              # 開始処理を表示
            page.snack_bar = ft.SnackBar(content=ft.Text(f"{tool_info['name']}を起動中..."))
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(f"ツール起動エラー: {str(ex)}"))
            page.snack_bar.open = True
            page.update()
    ...
    # レイアウト
    page.add(
        ft.AppBar(title=ft.Text("Flet マルチプラットフォーム開発ツール"), center_title=True),
        ft.Container(
            content=ft.Column([
                header,
                ft.Container(height=20),
                ft.Text("利用可能なツール", size=20, weight=ft.FontWeight.BOLD),
                ft.Text("以下のツールからプロジェクト開発に必要なものを選択してください"),
