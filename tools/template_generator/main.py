...
def main(page: ft.Page):
    ...
    customize_container = ft.Container(
        content=ft.Column([
            ft.Text("テンプレートを選択してください", italic=True)
        ]),
        padding=10,
        visible=False,
        ...
        tabs=[
            ft.Tab(
                ...
                content=ft.Container(
                    content=create_project_templates_view(templates["project_templates"]),
                    padding=10
                ),
            ),
            ft.Tab(
                ...
                content=ft.Container(
                    content=create_component_templates_view(templates["component_templates"]),
                    padding=10
                ),
            ),
            ft.Tab(
            ...
            card = ft.Card(
                content=ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(template["description"], size=12, color=ft.colors.BLACK54),
                        ft.Row([
                            ft.ElevatedButton("選択", on_click=lambda _, t=template: select_project_template(t))
                        ])
                    ]),
                    padding=10
                ),
            ...
            card = ft.Card(
                content=ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(template["description"], size=12, color=ft.colors.BLACK54),
                        ft.Row([
                            ft.ElevatedButton("選択", on_click=lambda _, t=template: select_component_template(t))
                        ])
                    ]),
                    padding=10
                ),
            ...
            card = ft.Card(
                content=ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(template["description"], size=12, color=ft.colors.BLACK54),
                        ft.Row([
                            ft.ElevatedButton("選択", on_click=lambda _, t=template: select_screen_template(t))
                        ])
                    ]),
                    padding=10
                ),
    ...
    def select_project_template(template):
        ...
        selected_template_info.value = f"選択されたテンプレート: {template['name']}\n{template['description']}"
    ...
    def update_customize_options(template_type, template):
        ...
        if template_type == "project":
            ...
            customize_container.content.controls.append(
                ft.Column([
                    ft.Text("プロジェクトカスタマイズ", size=16, weight=ft.FontWeight.BOLD),
                    ft.Divider(),
            ...
            customize_container.content.controls.append(
                ft.Column([
                    ft.Text("コンポーネントカスタマイズ", size=16, weight=ft.FontWeight.BOLD),
                    ft.Divider(),
                    component_name,
                    output_path
                ])
            ...
            screen_name = ft.TextField(
                label="画面名",
                width=300,
                hint_text="作成する画面名（PascalCase推奨）"
            )
            
            output_path = ft.TextField(
                label="出力パス",
                width=500,
                hint_text="views/ など、相対パスを入力"
            )
    ...
    generate_button = ft.ElevatedButton(
        "テンプレート生成",
        icon="create",
        on_click=lambda _: generate_template()
    )
    ...
    def generate_template():
        if not selected_template:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("テンプレートを選択してください"),
                action="閉じる"
            )
    ...
    def generate_project_template():
        if not project_name.value:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("プロジェクト名を入力してください"),
                action="閉じる"
            )
            page.snack_bar.open = True
            page.update()
        ...
        template_vars = {
            ...
            "app_description": project_description.value or "Fletを使用したアプリケーション"
        }
    ...
    def generate_component_template():
        ...
        if not component_name:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("コンポーネント名を入力してください"),
                action="閉じる"
            )
            page.snack_bar.open = True
            page.update()
