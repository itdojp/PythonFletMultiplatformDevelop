...
def main(page: ft.Page):
            ...
            try:
                ...
                try:
                    with Image.open(path) as img:
                        ...
                        info_text += f"ファイルサイズ: {os.path.getsize(path) // 1024} KB"
    ...
    def update_asset_scanner():
        ...
        for asset in assets_found:
            ...
            asset_list.controls.append(
                ft.ListTile(
                    leading=ft.Icon(
                        ft.icons.IMAGE if is_image_file(asset) else ft.icons.FONT_DOWNLOAD
                    ),
    ...
    def generate_assets(_):
        ...
        if not platform_selector.value:
            page.snack_bar = ft.SnackBar(content=ft.Text("プラットフォームを選択してください"))
            page.snack_bar.open = True
            page.update()
            return
        
        # 選択されたプラットフォームのデータを取得
        platform = next((p for p in platform_data if p["name"] == platform_selector.value), None)
        if not platform:
            return
        ...
        if asset_type_selector.value in ["アイコン", "画像"]:
            if not is_image_file(asset_path):
                page.snack_bar = ft.SnackBar(content=ft.Text("選択されたファイルは画像ではありません"))
                page.snack_bar.open = True
                page.update()
                return
    ...
    def process_image_asset(platform, src_path):
        try:
            ...
            # アセットタイプ
            asset_type = "icons" if asset_type_selector.value == "アイコン" else "images"
            
            # サイズ情報の取得
            sizes = platform["icon_sizes"] if asset_type_selector.value == "アイコン" else platform["screenshot_sizes"]
            ...
            with Image.open(src_path) as img:
                ...
                for size_info in sizes:
                    ...
                    if "scale" in size_info:
                        ...
                        # ディレクトリパス
                        dir_path = os.path.join(project_dir, f"assets/{platform['name'].lower()}/screenshots/{screen_type}")
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(f"アセット生成エラー: {str(e)}"))
            page.snack_bar.open = True
            page.update()
...
"""
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(f"フォント処理エラー: {str(e)}"))
            page.snack_bar.open = True
            page.update()
    ...
    def scan_asset_usage(_):
        if not working_dir.value or not os.path.exists(working_dir.value):
            page.snack_bar = ft.SnackBar(content=ft.Text("有効なプロジェクトディレクトリを指定してください"))
            page.snack_bar.open = True
            page.update()
            return
        ...
        if not asset_dirs:
            page.snack_bar = ft.SnackBar(content=ft.Text("アセットディレクトリが見つかりません"))
            page.snack_bar.open = True
            page.update()
            return
        ...
        for asset_dir in asset_dirs:
            for ext in ["png", "jpg", "jpeg", "svg", "gif", "webp", "ttf", "otf", "woff", "woff2"]:
                all_assets.extend(asset_dir.glob(f"**/*.{ext}"))
        
        if not all_assets:
            page.snack_bar = ft.SnackBar(content=ft.Text("アセットファイルが見つかりません"))
            page.snack_bar.open = True
            page.update()
            return
            ...
            try:
                with open(code_file, "r", encoding="utf-8") as f:
                    ...
                    for asset_path, asset_info in asset_usage.items():
                        ...
                        # ファイル名と相対パスの両方で検索
                        if asset_name in content or asset_path.replace("\\", "/") in content:
                            asset_info["used"] = True
                            asset_info["references"].append(str(code_file.relative_to(project_dir)))
            except Exception:
                # エンコーディングエラーなどをスキップ
                continue
        ...
        page.dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.Text(f"使用済みアセット: {len(used_assets)}", color=ft.colors.GREEN),
                ft.Text(f"未使用アセット: {len(unused_assets)}", color=ft.colors.RED if unused_assets else ft.colors.GREEN),
                ft.Divider(),
                ft.Text("未使用アセット:", weight=ft.FontWeight.BOLD) if unused_assets else ft.Text("すべてのアセットが使用されています！", color=ft.colors.GREEN),
                ft.ListView(
                    controls=[ft.Text(f"- {info['path']}") for info in unused_assets],
    ...
    page.add(
        ...
        ft.Container(
            content=ft.Column([
                ...
                ft.Row([
                    ft.Column([
                        ft.Text("アセットスキャン", weight=ft.FontWeight.BOLD, size=16),
                        ft.Row([
                            asset_type_selector,
                            scan_usage_button
                        ]),
                    ...
                    ft.Column([
                        ft.Text("アセットプレビュー", weight=ft.FontWeight.BOLD, size=16),
                        ft.Container(
