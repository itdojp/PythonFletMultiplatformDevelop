...
def main(page: ft.Page):
    ...
    project_tree = ft.TreeView(
        data=ft.TreeData(), width=250, height=400, on_select_change=on_file_selected
    )
    ...
    test_framework_dropdown = ft.Dropdown(label="テストフレームワーク", width=250)
    ...
    def update_framework_options(e):
        ...
        if test_type_dropdown.value:
            test_type = next(
                (
                    t
                    for t in test_templates["test_types"]
                    if t["name"] == test_type_dropdown.value
                ),
                None,
            )
            if test_type:
                ...
                if test_framework_dropdown.options:
                    test_framework_dropdown.value = test_framework_dropdown.options[
                        0
                    ].key
    ...
    create_test_dir = ft.Checkbox(label="テストディレクトリを作成", value=True)

    test_dir_name = ft.TextField(label="テストディレクトリ名", width=200, value="tests")

    # 選択された要素
    selected_elements = []

    # 選択された要素リスト
    selected_elements_list = ft.ListView(spacing=2, height=150, width=400)
    ...
    def scan_directory(dir_path, parent_id):
        try:
            ...
            files = [
                item
                for item in items
                if os.path.isfile(os.path.join(dir_path, item)) and item.endswith(".py")
            ]
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    function_item = ft.Row(
                        [
                            ft.Checkbox(
                                label=node.name,
                                on_change=lambda e, name=node.name, type="function": toggle_element_selection(
                                    e, name, type, file_path
                                ),
                            ),
                        ...
                        [
                            ft.Row(
                                [
                                    ft.Checkbox(
                                        label=node.name,
                                        on_change=lambda e, name=node.name, type="class", methods=methods: toggle_element_selection(
                                            e, name, type, file_path, methods
                                        ),
                                    ),
                            ...
                            if method != "__init__":  # 初期化メソッドは除外
                                method_item = ft.Row(
                                    [
                                        ft.Checkbox(
                                            label=method,
                                            on_change=lambda e, class_name=node.name, method_name=method: toggle_method_selection(
                                                e, class_name, method_name, file_path
                                            ),
                                        ),
                                        ft.Text(
                                            "メソッド", color=ft.colors.ORANGE, size=12
                                        ),
                                    ]
                                )
    ...
    def update_selected_elements_list():
        ...
        for element in selected_elements:
            ...
            if (
                element["type"] == "class"
                and "selected_methods" in element
                and element["selected_methods"]
            ):
    ...
    def generate_test_code(_):
        if not selected_elements:
            page.snack_bar = ft.SnackBar(
                content=ft.Text("テスト対象を選択してください"), action="閉じる"
            )
        ...
        framework = next(
            (
                f
                for f in test_type["frameworks"]
                if f["name"] == test_framework_dropdown.value
            ),
            None,
        )
        ...
        # 特殊なFletテスト処理
        if (
            test_type_dropdown.value == "UI テスト"
            and test_framework_dropdown.value == "Flet テスト"
        ):
            ...
            if any(e["type"] == "function" for e in selected_elements):
                # ページテスト
                template = next(
                    (t for t in flet_templates if t["name"] == "ページテスト"), None
                )
            elif any(e["type"] == "class" for e in selected_elements):
                # コントロールテスト
                template = next(
                    (t for t in flet_templates if t["name"] == "コントロールテスト"),
                    None,
                )
            ...
            if template:
                ...
                if element["type"] == "function":
                    test_code = template["template"].replace(
                        "{module_path}", module_path
                    )
                    test_code = test_code.replace("{function_name}", element["name"])
                    test_code = test_code.replace(
                        "{class_name}", pascal_case(element["name"])
                    )
            ...
            for element in selected_elements:
                if element["type"] == "function":
                    # 関数テスト
                    test_code = framework["template"].replace(
                        "{module_path}", module_path
                    )
                    ...
                    if "selected_methods" in element and element["selected_methods"]:
                        for method in element["selected_methods"]:
                            method_test_code = test_code.replace(
                                "{method_name}", method
                            )
                        ...
                        if "{method_name}" in test_code:
                            test_code = test_code.replace(
                                "{method_name}", "class_" + snake_case(element["name"])
                            )
                        code.append(test_code)
    ...
    page.add(
        ...
        ft.Row(
            [
                ft.Column([ft.Text("プロジェクト構造"), project_tree]),
                ft.Column(
                    [
                        selected_file_info,
                        ft.Text("関数"),
        ...
        ft.Divider(),
        ft.Text("テスト設定"),
        ft.Row([test_type_dropdown, test_framework_dropdown]),
        ft.Row([test_file_name, create_test_dir, test_dir_name]),
        ft.Divider(),
        ft.Text("選択された要素"),
        selected_elements_list,
        ft.ElevatedButton(
            "テストコード生成", icon=ft.icons.CODE, on_click=generate_test_code
        ),
    )
