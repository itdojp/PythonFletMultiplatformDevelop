...
def main(page: ft.Page):
    ...
    try:
        ...
        with open(os.path.join(script_dir, "components.json"), "r", encoding="utf-8") as file:
    ...
    def create_component_card(component):
        ...
        properties_chips = ft.Row(
            ...
            scroll="auto",
            controls=[
                ft.Chip(label=prop) for prop in component.get("properties", [])
            ]
        )
            ...
            content=ft.Column([
                ft.Row([
                    ...
                    ft.IconButton(
                        icon=ft.icons.COPY,
                        tooltip="コードをコピー",
                        on_click=lambda e, c=component["code"]: copy_code_to_clipboard(e, c)
                    )
                ]),
                ft.Container(
        ...
        return ft.Card(
            content=ft.Container(
                content=ft.Column([
                    ft.ListTile(
                        title=ft.Text(component["name"], size=20, weight=ft.FontWeight.BOLD),
    ...
    def filter_components(search_text):
        ...
        for component in components_data:
            ...
            if (search_text in component["name"].lower() or 
                search_text in component["description"].lower() or 
                any(search_text in prop.lower() for prop in component.get("properties", []))):
    ...
    header = ft.Container(
        content=ft.Column([
            ...
            ft.Text(
                "Fletで利用可能なUIコンポーネントとそのサンプルコード集",
                size=16
            )
        ]),
        padding=10,
