...
def main():
    parser = argparse.ArgumentParser(description="Fletテンプレートジェネレーター（CLI版）")
    ...
    generate_parser.add_argument(
        "--path", "-p", 
        help="出力パス（components/custom/ や views/ などの相対パス）"
    )
    generate_parser.add_argument(
        "--description", "-d", 
        help="説明（プロジェクトテンプレートのみ）"
    )
    ...
    if args.command == "list":
        ...
        if template is None:
            ...
            print("利用可能なテンプレート一覧を表示するには 'list' コマンドを使用してください。")
            return 1
