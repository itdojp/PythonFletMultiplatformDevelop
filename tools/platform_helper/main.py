...
def main(page: ft.Page):
    ...
    def generate_code():
        ...
        # 選択された機能のコードを取得
        feature_data = next(
            (f for f in platform_features if f["name"] == feature), None
        )
        if not feature_data:
            return
    ...
    copy_button = ft.ElevatedButton(
        text="コードをコピー", icon="copy", on_click=copy_to_clipboard
    )
    ...
    def save_to_file(_):
        ...
        def save_result(e: ft.FilePickerResultEvent):
            if e.path:
                try:
                    ...
                    page.snack_bar = ft.SnackBar(
                        content=ft.Text(f"コードを {e.path} に保存しました")
                    )
                    page.snack_bar.open = True
        ...
        file_dialog.save_file(
            allowed_extensions=["py"], initial_filename="platform_feature.py"
        )

    save_button = ft.ElevatedButton(
        text="ファイルに保存", icon="save", on_click=save_to_file
    )

    # レイアウト
    page.add(
        ft.AppBar(
            title=ft.Text("Flet プラットフォーム別機能実装ヘルパー"), center_title=True
        ),
        ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "以下から実装したい機能を選択すると、プラットフォーム別の条件分岐を含むコードが生成されます。"
                    ),
                    feature_dropdown,
                    ft.Container(height=20),
                    code_preview,
