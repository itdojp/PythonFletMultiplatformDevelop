...
DEVICE_PRESETS = [
    ...
    {"name": "モバイル (標準)", "width": 375, "height": 667, "icon": ft.icons.SMARTPHONE},
    ...
    {"name": "デスクトップ", "width": 1440, "height": 900, "icon": ft.icons.DESKTOP_WINDOWS},
...
if width < 600:  # モバイル
    ...
    def on_device_changed(e):
        ...
        selected_device = next((d for d in DEVICE_PRESETS if d["name"] == e.control.value), DEVICE_PRESETS[0])
    ...
    def apply_custom_size(e):
        try:
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text("有効な数値を入力してください"))
            page.snack_bar.open = True
            page.update()
    
    custom_size_button = ft.ElevatedButton(
        text="サイズ適用",
        on_click=apply_custom_size
    )
    ...
    def update_preview_size():
        ...
        preview_container.scale = max(0.3, scale * 0.9)  # 最低でも0.3、余裕を持って0.9倍
    ...
    def update_preview(e=None):
        ...
        try:
            ...
            if rules_editor.value:
                # ルールをコメント行から解析
                rule_lines = [line.strip() for line in rules_editor.value.split('\n') if not line.strip().startswith('#')]
                rule_code = '\n'.join(rule_lines)
            ...
            control = eval(layout_code, {
                "ft": ft, 
                "runs_count": layout_vars.get("runs_count", 2)
            })
        ...
        ft.Row([
            ...
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("レイアウト設定", weight=ft.FontWeight.BOLD, size=16),
                    layout_dropdown,
                    code_editor,
                    ft.Text("レスポンシブルール", weight=ft.FontWeight.BOLD, size=16),
                    ft.Text("サイズに応じた調整ロジック (Python構文):", size=12),
                    rules_editor,
                    update_button
                ]),
                width=450,
